...
main_loop(__rte_unused void *dummy)
{
	...
	// 	RTE_LOG(
	// 	    INFO, L3FWD, " -- lcoreid=%u portid=%u rxqueueid=%hhu\n", lcore_id, portid, queueid);
	// }
...
parse_config(const char *q_arg)
{
	...
	while ((p = strchr(p0, '(')) != NULL) {
		...
		// lcore_params_array[nb_lcore_params].queue_id = queues;
		lcore_params_array[nb_lcore_params].lcore_id = (uint8_t)int_fld[FLD_LCORE];
		// printf("port %u, queue %u, lcore %u\n",
...
// 			    s, nb_mbuf, MEMPOOL_CACHE_SIZE, 0, RTE_MBUF_DEFAULT_BUF_SIZE, socketid);
// 			if (pktmbuf_pool[socketid] == NULL)
...
main(int argc, char **argv)
{
	...
	RTE_ETH_FOREACH_DEV(portid)
	{
		...
		ret =
		    rte_eth_dev_configure(portid, (uint16_t)queues, (uint16_t)1, &local_port_conf);
		if (ret < 0)
		...
		// queues = reta_conf[0].reta[0] + 1;
		// if (queues != reta_conf[0].reta[0] + 1)
		// 	rte_exit(EXIT_FAILURE, "Cannot get correct number of queues: %d != %d\n", queues,
		// reta_conf[0].reta[0] + 1); printf("cms_rx_queue_per_lcore: %d\n", queues);
	...
	// 		ret               = rte_eth_rx_queue_setup(
	//             portid, queueid, nb_rxd, socketid, &rxq_conf, pktmbuf_pool[socketid]);
	// 		if (ret < 0)
	// 			rte_exit(EXIT_FAILURE,
